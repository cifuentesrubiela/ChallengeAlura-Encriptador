...
function encriptar() {
  const obtiene_el_elemento_de_entrada_text = document
    .getElementById("text")
    .value.toLowerCase();
  ...
  for (let i = 0; i < Object.keys(tablaEncriptar).length; i++) {
    ...
    almacenaResultadoCifrado = almacenaResultadoCifrado.replace(
      regex,
      tablaEncriptar[cifrar_la_palabra]
    );
  }
...
function desencriptar() {
  ...
  if (almacenaResultadoDesencriptado.match(/(enter|imes|ai|ober|ufat)/g)) {
    for (let i = 0; i < Object.keys(tablaDesencriptar).length; i++) {
      ...
      almacenaResultadoDesencriptado = almacenaResultadoDesencriptado.replace(
        regex,
        tablaDesencriptar[patron_buscar_desencriptar]
      );
    }
  ...
  document.getElementById("texto-vacio").textContent =
    almacenaResultadoDesencriptado;
  document.getElementById("copiar-codigo").style.display = "inline-block";
  muneco.style.display = "none";
}
